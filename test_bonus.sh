...
export PATH2=$PATH
export PATH="TOTO"
(unset PATH && echo ">$PATH<" && ./pipex input cat cat output)
RET1=$?
export PATH=$PATH2
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
...
export PATH="TOTO"
/bin/valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex input cat cat output 1>/dev/null 2>/dev/null
export PATH=$PATH2
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
...
echo -e "\033[1;37m ---------------------------------------------------------------------------------------------\033[0m"
echo -e "\033[1;37m [test 1] [basic test] \033[0m"
echo -e "./pipex input cat "grep toto" "wc -l" output
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
./pipex "input" "cat" "grep toto" "wc -l" "output"
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
...
valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex input cat "grep toto" "wc -l" output 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "./pipex input ls "grep toto" "wc -l" output
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
./pipex input cat "grep toto" "wc -l" output
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
...
valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex input cat "grep pipex" "wc -l" output 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "./pipex_bonus /dev/urandom "cat" "cat" "cat" "head -1" output
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
./pipex /dev/urandom "cat" "cat" "cat" "head -1" output
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
ps
cat output
valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex_bonus /dev/urandom "cat" "cat" "cat" "head -1" output 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "./pipex_bonus /dev/urandom "cat" "not" "cat" "head -1" output
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
./pipex /dev/urandom "cat" "not" "cat" "head -1" output
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
ps
cat output
valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex_bonus /dev/urandom "cat" "not" "cat" "head -1" output 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "./pipex input cat not not "ls" output
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
./pipex input cat not not "ls" output
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
...
valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex input cat not not "ls" output 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "( (echo "hello" ; echo "im not"; echo "life"; echo "EOF" ) | ./pipex here_doc EOF "cat" "wc -l" out );
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
( (echo "hello" ; echo "im not"; echo "life"; echo "EOF" ) | ./pipex here_doc EOF "cat" "wc -l" out );
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
echo -e "< input ls | grep pipex | wc -l > output2
\033[1;33m>----------------------------------------TRUE----------------------------------------------------"
...
((echo "hello" ; echo "im not"; echo "life"; echo "EOF" ) | valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex here_doc EOF "cat" "wc -l" out3 ); 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "( (echo "hello" ; echo "im not"; echo "life"; echo "EOF" ) | ./pipex here_doc EOF "cat" "not" out );
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
( (echo "hello" ; echo "im not"; echo "life"; echo "EOF" ) | ./pipex here_doc EOF "cat" "not" out );
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
...
( (echo "hello" ; echo "im not"; echo "life"; echo "EOF" ) | valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex here_doc EOF "cat" "not" out3 ); 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "( (echo "EOF " ; echo " EOF"; echo "E O F"; echo "EOF" ) | ./pipex here_doc EOF "cat" "cat" out );
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
( (echo "EOF " ; echo " EOF"; echo "E O F"; echo "EOF" ) | ./pipex here_doc EOF "cat" "cat" out );
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
...
diff out out2
 ...
 ( (echo "EOF " ; echo " EOF"; echo "E O F"; echo "EOF" ) | valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex here_doc EOF "cat" "cat" out3 ); 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "( (echo "EOF" ) | ./pipex here_doc EOF "cat" "cat" out );
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
( ( echo "EOF" ) | ./pipex here_doc EOF "cat" "cat" out );
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
...
( ( echo "EOF" ) | valgrind --log-file="leak" --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=240 --errors-for-leak-kinds=all ./pipex here_doc EOF "cat" "cat" out3 ); 1>/dev/null 2>/dev/null
if [  "$?" -ne "240" ]
then
echo -e "valgrind     : \033[32m[OK]\033[0m"
else
...
echo -e "( (echo "EOF " ; echo " EOF"; echo "E O F"; echo "EOF" ) | ./pipex here_doc EOF "yes" "head -10" out );
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
( (echo "EOF " ; echo " EOF"; echo "E O F"; echo "EOF" ) | ./pipex here_doc EOF "yes" "head -10" out );
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
...
echo -e "( (echo "EOF " ; echo " EOF"; echo "E O F"; echo "EOF" ) | ./pipex here_doc EOF "yes" "head -10" out );
\033[1;36m>----------------------------------------YOURS---------------------------------------------------"
( (echo "EOF " ; echo " EOF"; echo "E O F"; echo "EOF" ) | ./pipex here_doc EOF "yes" "head -10" out ) & ( (echo "EOF " ; echo " EOF"; echo "E O F"; echo "EOF" ) | ./pipex here_doc EOF "yes" "head -10" out );
RET1=$?
echo -e "-------------------------------------------------------------------------------------------------<\033[0m"
echo -e "yes << EOF | head -10 >> out2"
echo -e "return Value : \033[32m[OK]\033[0m"
